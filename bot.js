...
const LUIS_CONFIGURATION = 'InvestmentExplorer';
...
const BOND_ENQUIRY_BondSearch_INTENT = 'BondEnquiry_BondSearch';
const BOND_ENQUIRY_YieldRateSearch_INTENT = 'BondEnquiry_YieldRateSearch';
...
class BasicBot {
    ...
    async onTurn(context) {
        ...
        if (context.activity.type === ActivityTypes.Message) {
            ...
            ////William add entity extraction
            ////Note: outdated!!.......var entityData = results.luisResult.entities;
            //
            //Example: show me bond > 4%
            //         what is the current rate for 10 Yrs US Treasury
            console.log('Result: ' + Object.keys(results))
            console.log('Input: ' + results.text) 
            var intents = results.intents
            var intentData = Object.keys(intents)
            intentData.forEach(function(_intent) {
                var item = Object.keys(intents[_intent])  
                item.forEach(function(item_value) {
                    var _score = intents[_intent][item_value]
                    console.log('Intent: ' + intentData + ' | Score: '+ _score ) 
                })
            })

            var entityList = results.entities  
            var entityData = Object.keys(entityList)
            console.log('Entities found: ' + entityData) //$instance,greater_than,YeildRate
           
            var entity_name
            var entity_value
            var entity_yieldRate_value
            var entity_period_value
            var entity_bond_name
            var entity_greater_or_less_than
            //http://book.mixu.net/node/ch5.html
            entityData.forEach(function(entity){
                var item = Object.keys(entityList[entity])   
                entity_name = entity 
                item.forEach(function(item_value) {
                    entity_value = entityList[entity][item_value];
                    switch (entity_name) {
                        case 'YieldRate': 
                            entity_yieldRate_value = entity_value
                            break;
                        case 'Period': 
                            entity_period_value = entity_value
                            break;
                        case 'Bond': 
                            entity_bond_name = entity_value
                            break;
                        case 'greater_than': 
                            entity_greater_or_less_than = entity_value
                            break;
                        case 'less_than': 
                            entity_greater_or_less_than = entity_value
                            break;
                        default:
                            console.log("no such entity name...");
                    }
                    console.log('entity name: ' + entity_name + ' | value: ' + entity_value);
                  })
                }) 



            // Note: These sample code from document is outdated!!!!!!
            ...
            if (!dc.context.responded) {
                ...
                case DialogTurnStatus.empty:
                    ...
                    case GREETING_INTENT:
                        ...
                        break;
                    case BOND_ENQUIRY_BondSearch_INTENT:
                        await dc.context.sendActivity('Here are the recommend Bond: HSBC, AWS, MSFT ' +  'that has ' + entity_yieldRate_value + ' YTM'  );
                        break;
                    case BOND_ENQUIRY_YieldRateSearch_INTENT:
                        await dc.context.sendActivity( 'The current ' +  entity_bond_name + ' with Period: ' + entity_period_value  + ' YTM is 5%');
                        break;
                    case NONE_INTENT:
                    default:
